...
| INSTALL-PORT-003 | P2 | planned | Installer detects foreign process on engine instance port before binding — warns with actionable message instead of failing silently with EADDRINUSE | — | PR #101 done |
...
| INSTALL-TOKEN-OPT-SLOW-001 | P3 | planned | Linux install hangs for 60-120s during token-optimizer copilot-doctor check — blocks the entire install. The doctor check runs synchronously and has no timeout. | — | Add --timeout or skip doctor check during non-interactive install (--yes flag) |
| INSTALL-PROFILE-BACKUP-001 | P3 | planned | **myelin install should backup shell profile before writing** — Sessions have stripped user content (e.g. NVM setup) from `~/.zshrc` when the managed block is rewritten. Fix: before `writeFileSync(profilePath, updated)`, write a timestamped backup to `~/.myelin/backups/` and print the path so users can restore. | — | `writeFileSync(backupPath, existing); ok(\`backed up → ${backupPath}\`)` before the profile write in `install.mjs` |
