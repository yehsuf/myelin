...
| WIN-MITM-PATH-001 | P3 | **done** | Windows: verify shows Homebrew/pip mitmdump path instead of myelin-managed path | — | On Windows: myelin verify shows the Python pip-installed mitmdump (C:\Users\yehsuf\AppData\Roaming\Python\Python314\Scripts\mitmdump.exe) not the myelin-managed binary. This is the pip-installed version, not the pinned managed version. Should use the managed ~\.myelin\bin\mitmdump.exe. Also: RTK was not on PATH in SSH sessions (fixed this session by adding ~\.myelin\bin to HKCU\Environment PATH). |
| WIN-WINSW-ELEVATION-001 | P3 | **done** | myelin update fails on Windows because WinSW service registration requires elevation — orchestrator rolls back current.json every update | — | Windows myelin update always fails: WinSW start exit -1 (no elevation). Services run fine via myelin restart (uses registry Run key, no elevation needed), but the update orchestrator calls installWinswService which needs admin. Since myelin update is run via SSH (non-elevated), every update rolls back. Fix: (1) detect when WinSW is unavailable due to elevation and fall back to registry Run key for the update, OR (2) add a --skip-service-register flag that updates code but skips service re-registration (user manually runs myelin restart after). Workaround: manually activate current.json + myelin restart after each update. |
...
| --- | --- | --- | --- |
| STATS-COMBINED-001 | done | **`myelin stats` shows combined stats across primary + copilot instances** — After STATS-ISOLATION-001 separates the stats files, `myelin stats` must continue to show a single combined view (current UX, all savings on one page). CLI fetches `/stats` from all running engine instance ports and merges before rendering. Per-instance breakdowns accessible directly via HTTP (`curl :8787/stats` = Claude Code only, `curl :8788/stats` = Copilot only). Files: `src/cli/stats.mjs`. | feat(stats): combined primary+copilot view merged to main (2289361). mergeHeadroomLiteStats sums all numeric counters from both instances. Opus CR caught headroom-original shape incompatibility — fixed with allHeadroomLite guard that falls back to per-instance rendering for non-headroom-lite deployments. Live output: 'headroom (:8787 + :8788) — combined'. 1584/0. |
| UPDATE-ACTIVATION-001 | done | myelin update fails to activate new release when installer encounters launchctl conflict — stays on old version silently | Root cause fixed by fix/port-managed-detection (b5f0219): INSTALL-PORT-003 was flagging headroom_lite node processes as 'foreign' because lsof -F pc returns short name 'node'. Now supplements with ps -p <pid> -o command= for generic interpreters so .myelin paths in script args are visible to isHolderMyelinManaged. 1584/0 tests. Opus CR confirmed ps output on macOS doesn't truncate. Services will now be properly recognized as myelin-managed → launchctl bootstrap succeeds → orchestrator keeps activation → no more rollback. |
| WIN-CLAUDE-DISABLE-001 | done | Installer wires Claude Code settings on Windows even when Claude Code is not installed — wasted config + confusing | PR feat/win-claude-disable merged to main (4fb1562). Added integrations.claude_code config key (default true) + early readUserConfig check in install.mjs. Opus CR caught use-before-init ReferenceError (existingCfg at 2571 vs declaration at 2783), fixed with early raw readUserConfig. 1581/0. |
| PROXY-RESILIENCE-001 | done | **Copilot session disconnects when headroom-lite briefly restarts** — When the copilot headroom (:8788) crashes or restarts, mitmproxy gets ECONNREFUSED forwarding to it. The Copilot CLI retries only 5× over 6s before declaring failure and killing the session. Root cause observed: `[myelin] compression error: timed out` in mitmproxy log (compression timeout is 20s and handled gracefully), but brief headroom restart windows are not retried. Fix: in `copilot_addon.py`, when forwarding to `:8788` gets ECONNREFUSED, retry up to 3× with 1s sleep before returning an error to the client. Also add `MYELIN_HEADROOM_CONNECT_RETRIES` env var to tune. Separately: reduce watchdog `StartInterval` from 90s to 30s so dropped services are revived faster. | PR feat/proxy-resilience merged to main (141b94e). Added _headroom_urlopen_with_retry() with MYELIN_HEADROOM_CONNECT_RETRIES=3 env var. Launchd watchdog 90s→30s. Opus CR caught stale 90s in install.mjs + redundant import time, both fixed. 1581/0. |
| WIN-VERIFY-HEADROOM-001 | done | verify on Windows shows headroom-lite not running after switching to Python headroom — labels stale | Resolved: myelin restart fixed the labels — verify now shows 14/14 with 'Headroom service' (Python headroom) + correct watchdog entries. The stale headroom_lite labels appeared because WinSW installer removed old services before the new ones could start. myelin restart revived both headroom instances via the restart mechanism. |
| WIN-HEADROOM-ENGINE-001 | done | Switching Windows from headroom_lite to Python headroom is blocked — config change doesn't persist through loadConfig | Fixed by PR #114 (fix/config-set-raw-user): proxyAliasFor now respects explicit proxy.engine via ifUnset; setConfigValue uses readUserConfig to prevent baking derived fields. Windows now running Python headroom 0.32.1 on :8787/:8788. |
...
| HLITE-B5-PARAM-001 | P4 | **done** —  **OpenAI max_tokens → max_completion_tokens translation** — GPT-5/o-series reject legacy param. Upstream: fix(proxy/openai) GH #1774. Files: normalize/openai-params.mjs (new) |
| HLITE-B5-COST-001 | P4 | **done** —  **Cache write premium accounting** — subtract write premiums from net savings. Upstream: fix(proxy) GH #1800. Files: observability/ledger.mjs |
| HLITE-B6-CACHE-001 | P5 | **done** —  **Provider-agnostic cache delta + prefix** — cross-provider cache consistency metrics. Large effort. Upstream: feat(cache) GH #1868 |
...
| VERIFY-MITM-PATH-001 | P4 | **done** — **`myelin verify` reports wrong mitmproxy binary path** — `verify` resolves the mitmproxy path via `which mitmdump` (finds Homebrew at `/opt/homebrew/bin/mitmdump`) instead of reading the actual running process or plist. The real service uses the managed binary at `~/.myelin/components/mitmproxy/<ver>/bin/mitmdump`. Display-only bug — functionality unaffected. Fix: inspect plist `ProgramArguments` or resolve the PID executable directly instead of `which`. |
