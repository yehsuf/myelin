...
| WIN-HEADROOM-ENGINE-001 | P3 | planned | Switching Windows from headroom_lite to Python headroom is blocked — config change doesn't persist through loadConfig | — | On Windows, editing config.yaml to set proxy.engine: headroom is read back as headroom_lite by loadConfig. The normalizeCompressionEngine function reads proxy.engine:'headroom' from the raw file correctly, but the final merged config returned by loadConfig still has engine:'headroom_lite'. Likely caused by: (1) proxy.headroom_lite.enabled:true in the Windows config triggers the legacy override path, OR (2) deriveCanonicalCompression sets compression.backend which feeds back into engine resolution. Investigation blocked by SSH shell-escaping limitations. To fix: (a) also set proxy.headroom_lite.enabled:false in the same write, (b) debug what deriveCanonicalCompression does with a mixed config on Windows. |
| WIN-VERIFY-HEADROOM-001 | P3 | planned | verify on Windows shows headroom-lite not running after switching to Python headroom — labels stale | — | After switching proxy.engine from headroom_lite to headroom on Windows, myelin verify still checks for WinSW services named headroom_lite-primary and headroom_lite-copilot, which no longer exist. Python headroom is running healthy on :8787/:8788 but verify shows 8/14 with 6 failures. Fix: verify should adapt checks based on configured engine (headroom vs headroom_lite). Related: watchdog for Python headroom may also need setup. |
