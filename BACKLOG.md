...
| STATS-COMBINED-001 | P4 | planned | **Combined stats endpoint / CLI view across primary + copilot instances** — After STATS-ISOLATION-001 separates the stats files, add a way to view aggregate savings across both instances. Two options: (a) `myelin stats --combined` fetches `/stats` from both ports and merges in the CLI, (b) a new `/stats/combined` endpoint on mitmproxy that proxies and aggregates both. Option (a) is simpler and keeps headroom-lite unchanged. Files: `src/cli/stats.mjs`. | — | Implement after STATS-ISOLATION-001; prefer CLI-side merge (option a) |
