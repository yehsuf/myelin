...
| UPDATE-ACTIVATION-001 | P2 | planned | myelin update fails to activate new release when installer encounters launchctl conflict — stays on old version silently | — | On Mac: myelin update staged v1.3.6 but current.json was rolled back to v1.3.1 because the installer failed (launchctl bootstrap conflict with old copilot plist). Required manual current.json + force-restart to fix. Root causes: (1) update orchestrator rolls back current.json on any installer failure, (2) INSTALL-PORT-003 flags running headroom_lite node processes as 'foreign', triggering launchctl bootstrap failure. Fix: isHolderMyelinManaged should recognize node headroom-lite processes (by checking stateDir path), and installer should not fail hard on port conflicts for its own services. |
...
| WIN-CLAUDE-DISABLE-001 | P3 | planned | Installer wires Claude Code settings on Windows even when Claude Code is not installed — wasted config + confusing | — | Windows has no Claude Code installed but myelin install writes ANTHROPIC_BASE_URL etc. to ~/.claude/settings.json. Need a config flag (e.g. integrations.claude_code: false) to skip Claude Code wiring when not applicable. On Windows: since no Claude is installed, should skip ~/.claude/ setup entirely. |
