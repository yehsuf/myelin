...
| HLITE-B6-TOOLFLOOR-001 | P2 | planned | **Aggregate tool-output size floor so small Codex outputs compress** — individual tool outputs below MIN_ITEMS (9) are skipped, even when the aggregate batch is large. Ports upstream GH #2050/#2116. File: `compress/tool-output-compactor.mjs`. | — | Implement aggregate floor |

| WIN-HEADROOM-FALLBACK-001 | P2 | planned | **Auto-fallback to headroom-lite when headroom-original fails to install** — Currently when headroomOriginal (optional) fails during `myelin update`, the user is stuck with `backend: headroom-original` in config but no working binary. Feature: in the update orchestrator, after component staging, if the active backend's component was skipped (optional failure), automatically (a) update config to `headroom-lite`, (b) stage + activate headroomLite, (c) emit a clear warning: "headroom-original unavailable — switched to headroom-lite. To use headroom-original, install MSVC Build Tools and add `%USERPROFILE%\\.myelin\\components` to Windows Defender exclusions, then run `myelin update`." Files: `src/update/update-orchestrator.mjs`, `src/update/component-manifest.mjs`. | — | Design state machine for optional-component config migration |
| WIN-HEADROOM-BINONLY-001 | P3 | planned | **Use `--no-build` for uv pip install of headroomOriginal on Windows** — Add `--no-build` (or `--only-binary :all:`) to the uv pip install command for `uv-venv` components on Windows. This fails fast with "no binary wheel" instead of attempting a 2-minute Rust source build. Makes the error actionable (Defender is blocking the wheel) rather than confusing (MSVC not found). Files: `src/update/component-installers.mjs` → `buildComponentInstallPlan` case `uv-venv`. | — | Add platform-conditional flags to uv pip install args |
