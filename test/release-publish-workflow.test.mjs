...
test('publisher pins and verifies the headroom-lite v0.31.0 release before publishing', () => {
  const steps = loadPublishSteps();
  const pinCheck = findStepByName(steps, 'Verify headroom-lite v0.31.0 release exists');
  assert.match(pinCheck.run, /headroom-lite\/releases\/tags\/v0\.31\.0/);
});
