...
async function probeEngineInstance(instance, {
  ...
  if (instance.engine === 'headroom_lite') {
    ...
    results.push({
      name: `${label} health`,
      ok: healthy,
      detail: healthy ? `running — ${response.version ? `v${response.version}, ` : ''}mode: ${response.mode}` : 'not running — run: myelin restart',
    });
  } else {
...
export async function buildVerifyResults({
  ...
  if (includeMitmCheck && cfg.proxy?.mitm?.enabled) {
    ...
    let mitmdump = null;
    try {
      const componentsRoot = join(managedPaths({ home: homedir() }).root, 'components');
